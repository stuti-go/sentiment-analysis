...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    average_hinge_loss = total_hinge_loss / n
...
def perceptron_single_step_update(
    ...
    """
    prediction = np.sign(np.dot(current_theta, feature_vector) + current_theta_0)
    if prediction != label:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    avg_theta = sum_theta / (n_samples * T)
    avg_theta_0 = sum_theta_0 / (n_samples * T)
...
def pegasos(feature_matrix, labels, T, L):
    ...
    """
    n_samples, n_features = np.shape(feature_matrix)
    theta = np.zeros(n_features)
    theta_0 = 0.0
    t = 0
...
def classify(feature_matrix, theta, theta_0):
    ...
    tol=0
    classifications = np.array([-1 if score <= tol else 1 for score in scores])
